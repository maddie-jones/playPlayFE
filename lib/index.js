...
function updateFavorite(e) {
  ...
  fetch(`http://localhost:3000/api/v1/favorites/${this.dataset.favid}`, {
    method: 'PUT',
...
function populateFavorites(favorites) {
  favorites.forEach((favorite, index) => {
    ...
    favDiv.setAttribute('data-artist_name', `${favorite.artist_name}`);
    favDiv.setAttribute('data-genre', `${favorite.genre}`);
    favDiv.setAttribute('data-rating', `${favorite.rating}`);

    const favHtml = `
      <li class="fav-name">Name: ${favorite.name}</li>
      <li class="fav-artist_name">Artist Name: ${favorite.artist_name}</li>
      <li class="fav-genre">Genre: ${favorite.genre}</li>
      <li class="fav-rating">Rating: ${favorite.rating}</li>
...
function showFavorites(e) {
  if (event.target.classList[0] === 'playlist-names') {
    const favoriteIds = event.target.dataset.favorites.split(",");
    favoritesIds.reduce((id) => {
...
function addFavorite(index) {
  let childElements = document.getElementById(`song-${index}`).childNodes
  let favTitle = childElements[1].textContent.slice(7,50)
  let favArtist = childElements[3].textContent.slice(7,50)
  let favGenre = childElements[5].textContent.slice(7,50)
  let favRating = childElements[7].textContent.slice(7,50)
  postFavorite(favTitle, favArtist, favGenre, favRating)
}
